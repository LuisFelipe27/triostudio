...
class PaginationFormSetBase(object):
    ...
    def make_filters(self):
        query = self.request.GET.get('query_paginator', '')
        filters = ''
        for field in self.search_fields:
            if filters:
                filters += ' | '
            filters += f"Q({field}__icontains='{query}')"
    ...
    def mount_paginator(self, page_num: int = None):
        ...
        if self.search_fields and query:
            self.queryset = self.queryset.filter(self.make_filters())
...
class TabularInlinePaginated(TabularInline):
    ...
    def get_formset(self, request, obj=None, **kwargs):
        ...
        PaginationFormSet.search_fields = self.search_fields
